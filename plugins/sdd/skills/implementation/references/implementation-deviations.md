...
- `CONTRACT_DEVIATION`: altera operação, endpoint/method, request/response, schema, error, versioning, idempotência, paginação, compatibility ou event semantics. Bloqueie e retorne a `$api-designer`; considere `$adr-author` se a mudança for arquiteturalmente material.
